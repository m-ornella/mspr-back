...
def create_answer_endpoint(answer: AnswerCreate, db: Session = Depends(SessionLocal)):
    return create_answer(db, answer)

@router.get("/{answer_id}", response_model=Answer)
def read_answer(answer_id: int, db: Session = Depends(SessionLocal)):
...
def read_answers(skip: int = 0, limit: int = 100, db: Session = Depends(SessionLocal)):
    return get_answers(db, skip, limit)

@router.put("/{answer_id}", response_model=Answer)
def update_answer_endpoint(answer_id: int, answer: AnswerUpdate, db: Session = Depends(SessionLocal)):
...
def delete_answer_endpoint(answer_id: int, db: Session = Depends(SessionLocal)):
